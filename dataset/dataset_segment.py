...
class SegmentationDataset(Dataset):
    ...
    def __init__(self, img_root, bbox_root, seg_root, feature_root, pos_root, transform, phase, interval, occl_thres):
        ...
        for dir_ in dir_list:
            ...
            if (interval[0] <= dir_id < interval[1]):
                ...
                for img_id in os.listdir(path.join(img_root, dir_)):
                    ...
                    if path.exists(path.join(feature_root, "video_{:04d}".format(dir_id), str(idx)+'.pt')) and \
                       path.exists(path.join(pos_root, "video_{:04d}".format(dir_id), str(idx)+'.pt')):
                        continue
                    peds = [ped for ped in bbox["objLists"][idx-1] if ped['occl'][0] < occl_thres]
                    for ped in peds:
                    ...
                    'pos_out':path.join(pos_root, "video_{:04d}".format(dir_id), str(idx)+'.pt'), \
                    'idx':idx, 'dir':dir_id})
                if len(ped_dict) == 0:
                    print(ped_dict)
                    continue
